...
public:
    ...
    void detect_raw_lines(const cv::Mat &gray_img,
                          std::vector<std::vector<KeyLine>> &keyline_octaves);  // without line thresholding, multi-octaves
    void detect_raw_lines(const cv::Mat &gray_img, cv::Mat &linesmat_out,
                          bool downsample_img = false);  /* output is n*4 32F mat. x1 y1 x2 y2 */
    ...
    void detect_descrip_lines_octaves(const cv::Mat &gray_img, std::vector<std::vector<KeyLine>> &keylines_out,
    ...
    void match_line_descrip(const cv::Mat &descrips_query, const cv::Mat &descrips_train,
                            std::vector<cv::DMatch> &good_matches, float matching_dist_thres = 25);
