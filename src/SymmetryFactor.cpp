...
namespace gtsam_soslam
{
            ...
            boost::optional<gtsam::Matrix &> H2) const
    {
        ...
        try
        {
            ...
            // std::cout << feature_points.size() << std::endl;

            // uniform sampled points in 2D plane
            // x, y in image form
            std::vector<pair<double, double>> uniform_sample_points;
            int x_step = bounding_box_width / 5;
            int y_step = bounding_box_height / 5;
            ...
            for (const auto &[i, j] : uniform_sample_points)
            {
                ...
                double min_dis = -1;
                pair<double, double> nearest_edge_point;
                ...
                gtsam::Vector3 edge_2D(nearest_edge_point.first, nearest_edge_point.second, 1);
                ...
                Eigen::VectorXd quadric_pose(3);
                quadric_pose << quadric.pose().x(), quadric.pose().y(), quadric.pose().z();
                double d = -quadric_pose.dot(horizon.head(3));
                ...
                gtsam::Matrix34 extrinsic = I34 * pose.matrix();
                ...
                // gtsam::Matrix3 camera_T_image = (calibration_->K()).inverse();
                // gtsam::Matrix4 world_T_camera = pose.matrix();
                // gtsam::Matrix34 image_T_world = image_T_camera * (world_T_camera).block(0, 0, 3, 4);
                symmetry_sample_2D = world2image * symmetry_sample_3D;
                double symmtery_sample_2D_x = symmetry_sample_2D[0] / symmetry_sample_2D[2];
                double symmtery_sample_2D_y = symmetry_sample_2D[1] / symmetry_sample_2D[2];
